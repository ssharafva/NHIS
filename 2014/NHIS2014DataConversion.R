...
tbl<-read.csv("./NHIS/2014/personx_summary.csv")

#Reading the original fixed width file 
clmnlen<-tbl$Length
clmnnme<-tbl$FinalDocName
system.time(data<-read.fwf(unz("./NHIS/2014/personsx.zip","personsx.dat"),
                           clmnlen,col.names=clmnnme))

#Saving the data in RDS format that can be read in a fraction of a second
saveRDS(data, file = "./NHIS/2014/personsx.rds")
system.time(x<-readRDS("./NHIS/2014/personsx.rds"))
